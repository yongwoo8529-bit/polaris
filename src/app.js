...
function explodeToGalaxies(specialType) {
    ...
    setTimeout(() => {
        ...
        document.getElementById("result-desc").innerText = "은하 가운데를 클릭해 설명을 읽어보세요";
...
const galaxyDetails = {
    1: {
        ...
        tagline: "왜 그럴까 — 질문이 멈추지 않는 사람",
        core: "분석적 사고와 강한 지적 호기심이 핵심 동력입니다. 현상의 표면을 넘어 원리와 구조를 탐구하려는 성향이 뚜렷하며, 관심 주제에 대해 깊이 있게 파고드는 집요함이 두드러집니다.",
        moment: "복잡한 문제의 실마리를 스스로 찾아냈을 때, 또는 새로운 지식 체계를 이해하고 정리했을 때 강한 성취감을 느낍니다. 지식의 경계를 확장하는 탐구 환경에서 자연스러운 몰입이 이루어집니다.",
    ...
    2: {
        ...
        tagline: "계획 없이 시작하면 마음이 불안한 사람",
        core: "체계적 사고와 논리적 구조화 능력이 강점입니다. 복잡한 문제를 단계적으로 분해하고 효율적인 시스템을 설계하는 데 탁월하며, 완성도 높은 결과물을 만들어내는 데서 동기를 얻습니다.",
        moment: "계획이 정확하게 실행되거나 복잡한 체계가 맞물려 돌아갈 때 강한 성취감을 느낍니다. 예측 가능하고 안정적인 환경에서 집중력이 극대화되며, 기준과 절차를 명확히 세우는 과정에서 역량이 빛납니다.",
    ...
    3: {
        ...
        tagline: "옆 사람 표정 변화를 먼저 알아채는 사람",
        core: "높은 감수성과 공감 능력이 핵심 자산입니다. 타인의 감정 변화를 세밀하게 감지하고, 관계 속에서 자연스럽게 안정감을 조성하는 역할을 수행합니다. 사람 중심의 환경에서 두드러진 역량을 발휘합니다.",
        moment: "누군가가 자신의 감정을 진심으로 이해받았다고 느낄 때, 또는 어려운 상황에 처한 이를 실질적으로 도왔을 때 깊은 보람을 느낍니다. 관계의 질을 높이는 과정에서 자연스러운 몰입이 이루어집니다.",
    ...
    4: {
        ...
        tagline: "만들고 표현해야 에너지가 차는 사람",
        core: "독창적인 표현력과 창조적 관점이 핵심입니다. 경험과 감정을 고유한 형식으로 외화(外化)하려는 내적 동기가 강하며, 기존의 틀을 벗어난 방식으로 세계를 재해석하는 데서 강점을 발휘합니다.",
        moment: "자신의 표현이 타인에게 감동이나 영감을 줄 때, 또는 창작 과정에서 완전한 몰입을 경험할 때 가장 강한 에너지를 발휘합니다. 결과물이 세상과 연결될 때 깊은 존재감을 느낍니다.",
    ...
    5: {
        ...
        tagline: "맡은 건 끝까지 — 신뢰가 가장 큰 가치인 사람",
        core: "높은 책임감과 안정 지향적 성향이 강점입니다. 공동체의 균형을 유지하고 구성원이 신뢰할 수 있는 기반을 만드는 역할에서 두드러진 역량을 발휘하며, 지속성과 신뢰가 핵심 가치입니다.",
        moment: "맡은 역할을 끝까지 완수하거나 팀 전체의 목표 달성에 기여했을 때 깊은 만족감을 느낍니다. 신뢰를 기반으로 한 안정적인 관계에서 집중력이 극대화되며, 꾸준함으로 쌓은 성과에서 가장 큰 보람을 얻습니다.",
    ...
    6: {
        ...
        tagline: "직접 해봐야 이해되고, 만들어야 직성이 풀리는 사람",
    ...
    7: {
        ...
        tagline: "딱 하나로 정의되기엔 가능성이 너무 많은 사람",
    ...
    8: {
        ...
        tagline: "지금 방향을 찾는 중 — 정해지지 않은 것이 가능성",
        core: "현재 자신의 방향성을 깊이 성찰하는 단계에 있습니다. 외부의 기준보다 내면의 기준을 중시하는 성향이 강하며, 쉽게 흔들리지 않는 독립적인 판단력을 지니고 있습니다.",
        moment: "외부의 평가에 흔들리지 않고 스스로의 기준으로 선택을 내릴 때, 또는 타인이 지나쳤던 깊은 통찰을 발견했을 때 자신만의 가치를 확인합니다. 침묵 속의 성찰이 단단한 방향성으로 이어집니다.",
        jobs: ""
    }
};
